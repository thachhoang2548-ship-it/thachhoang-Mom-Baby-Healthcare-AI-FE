...
import { Users, Lock, Unlock, UserPlus, ShieldAlert, FileText, Settings, CheckCircle2 } from "lucide-react";
          ...
          <div className="bg-white dark:bg-gray-800 rounded-3xl p-6 border border-gray-100 dark:border-gray-700 space-y-4">
            {businessRules.length === 0 ? (
              ...
              businessRules.map((rule) => (
                <div key={rule.id} className="p-4 rounded-2xl border border-gray-100 dark:border-gray-700 flex justify-between items-center">
                  <div>
                    <h4 className="font-bold text-xs text-gray-900 dark:text-white">{rule.ruleName || rule.name}</h4>
                    <p className="text-[11px] text-gray-400 mt-0.5">{rule.description}</p>
                  </div>
                  <span className="text-xs font-black text-blue-600 bg-blue-50 px-3 py-1 rounded-full">{rule.value || "Active"}</span>
                </div>
              ))
            )}
          </div>
