...
export const useRecipeStore = create((set, get) => ({
  ...
  fetchCurrentProfile: async () => {
    try {
      ...
      console.warn("Could not fetch current profile (perhaps no lifestyle entry yet):", err.message);
    }
  },
  ...
  generateRecipes: async (preferences) => {
    ...
    try {
      ...
      set({ error: errMsg });
      // Toaster is already triggered in service, so we just propagate
      throw err;
    } finally {
  ...
  toggleSave: async (recipeId) => {
    try {
      ...
      if (res.success && res.data) {
        // Update local state isSaved status
